...
class Explore extends Component<IProps> {
  state = {
    ...
    return (
      <Block animated middle flex={searchFocus} style={styles.search}>
        <Input
          ...
          onChangeText={(text: any) => this.setState({ searchString: text })}
          value={searchString}
          onRightPress={() =>
    ...
    return (
      <Block style={{ marginBottom: height / 3 }}>
        ...
        <Block row space="between" wrap>
          {images.slice(1).map((img: any, index: any) => this.renderImage(img, index))}
        </Block>
      </Block>
    );
  }
